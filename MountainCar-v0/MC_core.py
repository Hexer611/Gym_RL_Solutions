...
class my_AI:
    def __init__(self, chk_dir):
        ...
        parallel_train_env = batched_py_environment.BatchedPyEnvironment([suite_gym.load(env_name) for _ in range(self.num_parallel_envs)])
    ...
    def train(self):
        ...
        for _ in range(5000):
            ...
            if self.total_step % 10000 == 0:
                self.test_agent(tries=5, display_freq=100)
                print('Brain saved.')
                self.checkpointer_save()
