...
import numpy as np
from tf_agents.environments import suite_gym, tf_py_environment, batched_py_environment, py_environment
...
class my_AI:
    def __init__(self, chk_dir):
        ...
        train_env = batched_py_environment.BatchedPyEnvironment([suite_gym.load(env_name) for _ in range(self.num_parallel_envs)])
    ...
    def create_ppo_agent(self, learning_rate):
        ...
        agent = ppo_clip_agent.PPOClipAgent(
            ...
            num_epochs=3,
            entropy_regularization=40,
            reward_norm_clipping=0.5)
    ...
    def train(self):
        ...
        for _ in range(5000):
            ...
            print('Episode {0} - Total Steps {1} - Average Reward {2} - Loss {3}'.format(
                self.total_episode, self.total_step, train_reward * self.num_parallel_envs, loss / iterations))
...
one.test_agent(tries=100, display_freq=100)
